...
- Próximo: Sprint 1 cierre técnico pendiente de push a `main` — arrancar Sprint 2 (cerrojo MEGA + LED RGB + pantallas Compose) una vez CI verde.
- CI 2026-08-26: `ci.yml:107` job `docker-build` mejorado a `Docker Build + Integration Test` — espera `postgres healthy` (`docker inspect`), poll `GET /health` 30×2s + `grep '"status":"ok"'`, luego integración `POST`/`GET` 4 endpoints (`/api/access-events` HU-01, `/sensor-events` HU-03, `/security-events` HU-02, `/rover/telemetry` RF-3.3) y `GET /`; `if: failure()` vuelca `docker compose logs` + `if: always()` `down -v`. Validado local `2026-08-26`: `docker compose build` OK 32s, `postgres Healthy` en 12s, `FastAPI {"status":"ok","database":"ok"}` en 6s, 4 POST 201 + 4 GET 200 OK.
