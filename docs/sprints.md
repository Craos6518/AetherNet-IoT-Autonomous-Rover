...
> **Nota:** este archivo describe la planeación _original_. Si el equipo se desvía de fechas o alcance, actualízalo aquí — un agente que lea un sprint desactualizado puede proponer trabajo que ya no aplica o saltarse dependencias reales.
...
- Sprint activo: **Sprint 3 — validación del rover, telemetría y control remoto en tiempo real**.
- Estado general: **Sprint 1 y Sprint 2 consolidados documentalmente**; la integración del rover, la capa de alarmas y la sincronización con la app se han alineado con el repositorio actual.
- Enfoque vigente: robustecer el enlace RF, la telemetría, la evasión autónoma y la sincronización de la app con el backend local.
- Telegram directo: **activo** como notificación de intrusión; Node-RED queda como referencia técnica y no como capa operativa activa.
- Tuya: **cancelado** y explicitado como fuera de alcance.
- Notebook y documentación: **sincronizados** con el estado real del proyecto para evitar drift entre código, investigación y entregables académicos.

### Actualización 2026-09-14

- El alcance del proyecto refleja la solución real implementada: acceso local + alarma + rover + notificaciones directas.
- La app Android y el backend forman la capa de coordinación, mientras que los microcontroladores mantienen la lógica crítica y local.
- El rover sigue siendo el centro del trabajo de Sprint 3 y se documenta como plataforma de validación de sensores, EMA y control por RF.
- Está definido el valor de continuidad de la documentación: si cambia un alcance, se debe actualizar el backlog, el riesgo, la matriz de sprint y los notebooks asociados.

### Estado por área

- Móviles: app funcional de dashboard + joystick + PIN + telemetría.
- Backend: FastAPI + PostgreSQL + Mosquitto operando como coordinación local.
- Firmware: MEGA y Rover con validaciones y alarmas base aplicadas.
- Seguridad: LED RGB local + Telegram directo, sin dependencia de Tuya / cloud.
- Estadística: EMA, análisis externo y datasets ya integrados en la documentación.

### Cierre del sprint y criterio de actualización

Durante cada cierre de sprint, se debe revisar y actualizar:

1. `docs/backlog.md` con el estado de cada tarea.
2. `docs/risk-register.md` con severidad y mitigación.
3. `docs/README.md` y `README.md` con alcance actual.
4. notebooks clave con un resumen del estado del proyecto.
5. trazabilidad HU/RF/RNF para asegurar que no haya drift conceptual.

- **2026-09-11 14:28 — TCRT/HC-SR04 ✅ Done `e821700` (pista blanca pot mitad 3mm):** `rover-uno 6832b` `analogRead >500 BORDE` `A0 trasero/A1 izq/A2 der` invertido (blanca baja 290 OK vs negra/vacío alta 850 BORDE) `test-rover-sensors 4712b` `A0 845 OK negra` `A0 290 BORDE blanca` validado `5mm` + `HC-SR04 TRIG2 ECHO3 EMA α=0.2` `US_raw 6-33cm EMA 6-32cm` `OBSTACLE 30cm` + telemetría `T_telemetry_invertida 89` `ir_left true/false ir_right true/false` `ultrasonic 0-95cm` `rf_rssi -70` `mode 1/2`. **Calce Rover:** TT 6V 1:48. **Pin 16↔16 17↔17** invertido. **Sensores:** `KY-008` Sprint 2 Done, `nRF24` Sprint 1 Done, `EST-02` EMA ✅ adelantado Sprint 3, `EST-03` KY-037 pendiente Sprint 4.
- **2026-09-11 09:43 — MOV-05/06 RF-1.2 ✅ Done `3ca12c9` (11h 23:00 + 07:00-09:29):** `JoystickScreen Canvas 120dp` `JoystickMapper deadband 60` `ViewModel throttle 50ms` `MqttManager QoS0 aethernet/rover/command:69` → `gateway-esp32 CE5 CSN15 Ch76 2MBPS` `RF TX 250→249 99.6%` → `rover-uno CE4 CSN10 ENA5/ENB11 6900b` `RF RX 232` + `FAIL-SAFE 500ms HU-04` + `aethernet/rover/telemetry rf_rssi -70` — validado `mosquitto_sub 563 líneas` `T3 33K` `T4 7.3K` `T2_mega 1.5K` `docker health ok` — `assembleDebug` + `JoystickMapperTest 10` + `ViewModelTest 6` verdes. **Calce Rover:** chasis TT 6V 1:48 ×4 `hardware-inventory.md:36` + `rover-uno.ino:6,89` `MIN_PWM 60` TT. **Notebook:** `docs/Firmware/notebook/Firmware_Notebook.ipynb` 16 celdas + `docs/logs/firmware_sprint3/` 20 logs.
...
- **2026-09-09:** `ec5d70b` docs(admin,estadística): `Proyecto_AetherNet_Andres_Felipe_Martinez_Henao.docx` (99KB, 14 secciones, WBS 40 paquetes, costos COP 176k/1.176M) + `docs/Administracion-Proyectos/README.md` + `docs/Estadistica/CONTRASTE_Admin_vs_Estadistica.md` + bench 36.5k filas (`water_turbidity 31.5k` + `gesture 5k`) + `stats/water_turbidity_analysis.py:1` 364 líneas + `stats/data/water_turbidity_report.json` + PNGs `water_us_vs_true.png`/`water_ir_by_angle.png`
