...
| MOV-05 | Joystick virtual en Compose (captura de vectores X,Y) — ✅ Done `feature/app-joystick-virtual@3ca12c9` 2026-09-11 `JoystickScreen Canvas 120dp` `JoystickMapper X,Y -1..1` `JoystickUiState` verificado `mosquitto_sub 251 cmds` + `assembleDebug` | M | 3 | — | RF-1.2 |
| MOV-06 | Envío de comandos del joystick con baja latencia (throttling/debounce) — ✅ Done `feature/app-joystick-virtual@3ca12c9` 2026-09-11 `ViewModel throttle 50ms 20Hz` `Mapper deadband 60` `MqttManager QoS0 aethernet/rover/command:69` → `gateway CE5 CSN15 RF TX 250→249 99.6%` → `rover CE4 CSN10 RF RX 232` `FAIL-SAFE 500ms HU-04` `T3 33K T4 7.3K` `JoystickMapperTest 10 ViewModelTest 6` | M | 3 | MOV-05, RF-2.1 operativo | RF-1.2 |
...
| EST-06 | Análisis descriptivo (medias, varianzas, gráficas) de sensores — 🔄 Adelantado parcial: `stats/water_turbidity_analysis.py` + `stats/data/water_turbidity_report.json` + `water_us_vs_true.png`/`water_ir_by_angle.png` + `docs/Estadistica/notebook/EMA_Estadistica.ipynb` §7c sobre 36.5k filas (water 31.5k + gesture 5k, CC BY-SA 4.0/CC0) — descriptivo completo Sprint 4 con datos propios | S | 4 | EST-04 | RNF-2.2 |
