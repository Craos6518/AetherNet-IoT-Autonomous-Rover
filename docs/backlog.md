...
| MOV-05 | Joystick virtual en Compose (captura de vectores X,Y) — ✅ Done Sprint 3 `feature/app-joystick-virtual` 2026-09-11 (RF-1.2: `JoystickScreen` Compose Canvas + `JoystickViewModel` vectores X,Y normalizados -1..1 → `left_pwm/right_pwm` -255..255 tank-steering, `MqttManager.publishRoverCommand` `aethernet/rover/command:69` → Gateway `handleRoverCommand:241` → `radio.write` nRF24L01 → `rover-uno.ino:219` `handleRfCommands`) | M | 3 | — | RF-1.2 |
| MOV-06 | Envío de comandos del joystick con baja latencia (throttling/debounce) — ✅ Done Sprint 3 `feature/app-joystick-latency` 2026-09-11 (RF-1.2 <10ms RF / <50ms `prd.md:50`: throttle 50ms + deadband `MIN_PWM 60` espejo `rover-uno.ino:70`, `gateway-esp32.ino:241` `stopListening→write→startListening`, `rover-uno.ino:212` loop 100Hz + `MqttManager` Paho ws://9001 fallback) | M | 3 | MOV-05, RF-2.1 operativo | RF-1.2 |
| MOV-07 | Fallback Bluetooth SPP para nodos críticos si cae el Wi-Fi — ✅ Done Sprint 3 `feature/app-bluetooth-fallback` 2026-09-11 (RF-1.3: `BluetoothManager` SPP `HC-06` `BLUETOOTH_CONNECT` runtime perm, `BluetoothScreen` scan/pair + `CMD:ACCESS` fallback si `MqttConnectionState.Disconnected`, `MEGA` sin cambios — `uart_protocol.cpp:40` ya acepta `CMD:ACCESS`) | S | 3 | Nodo Acceso Compacto (HC-06) | RF-1.3 |
