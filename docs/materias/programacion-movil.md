...
| Colecciones | `Map<String, TuyaState>` / estados de bombillo y LED local; colecciones inmutables expuestas al estado de UI | `DashboardViewModel.kt:13` |
...
| StackLayout / Grid | Equivalentes Compose: `Column`/`Row` para layout lineal (dashboard de luces), `LazyVerticalGrid` si la grilla lo requiere (RF-1.1 Dashboard) | Por construir — backlog MOV-02/MOV-08 |
| Views / controles | Controles planificados: control bombillo Tuya (RF-1.1), **joystick virtual custom** capturando vectores X,Y (RF-1.2), indicadores de telemetría | Backlog MOV-02, MOV-05, MOV-08 |
...
| Estado de UI modelado | `sealed interface DashboardUiState` con 4 estados exhaustivos: `Connected(roverTelemetry, lastAccessEvent)`, `Disconnected(reason)`, `Connecting`, `Error(message)` — la UI no puede quedar en un estado indefinido | `DashboardViewModel.kt:11-23` |
...
| Conexiones / tiempo real | Suscripción a topics de telemetría (`aethernet/rover/...`, `aethernet/access/pin`, `aethernet/tuya/command` definidos como TODOs contractuales) | `DashboardViewModel.kt:65-75` |
...
| Acceso al hardware del dispositivo | **Bluetooth SPP** para contingencia si cae Wi-Fi (RF-1.3, backlog MOV-07); el "hardware" remoto (Rover, bombillo Tuya, cerrojo) se controla vía MQTT→ESP32 | `docs/requirements.md` RF-1.3; `docs/hardware-inventory.md` (HC-06) |
...
- **Sprints:** Sprint 2 (pantallas luces/bombillo) → Sprint 3 (joystick+telemetría) → Sprint 4 (dashboard consolidado + tests)
