...
| **Indicador Visual Local** | **LED RGB (ánodo/cátodo común) conectado a Arduino MEGA, pines PWM `44(R)/45(G)/46(B)` junto al teclado 4x4/servo — ver § Detalle de pines** | **Feedback físico inmediato del estado de acceso, sin depender de red: verde sincronizado a ventana 5 s de desbloqueo (HU-01), rojo remoto vía Gateway en fallo PIN / intrusión láser (HU-02). No requiere Wi-Fi ni MQTT para el verde local.** |
...
- **LED RGB local:** `44(R),45(G),46(B)` PWM — HU-01 verde sólido mientras `doorUnlocked` (ventana `DOOR_AUTO_LOCK_MS=5000`), OFF al re-bloquear; rojo 1s en PIN erróneo, azul 50ms por dígito (no bloqueante).
